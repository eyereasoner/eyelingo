...
type ComplexExampleInput struct {
	Question  string
	Exponents []ComplexExponentInput
	Inverses  []ComplexInverseInput
}

type ComplexExponentInput struct {
	ID         string
	Expression string
	Base       Complex
	Power      Complex
...
type ComplexInverseInput struct {
	ID         string
	Operation  string
	Expression string
	Input      Complex
...
func derive() Analysis {
	defaults := ComplexExampleInput{
		Question: "Evaluate the complex-number facts queried by complex.n3.",
		Exponents: []ComplexExponentInput{
			{
				ID:         "C1",
				Expression: "((-1 0) (0.5 0)) complex:exponentiation ?C1",
				Base:       Complex{-1, 0},
				Power:      Complex{0.5, 0},
			...
			{
				ID:         "C2",
				Expression: "((e 0) (0 pi)) complex:exponentiation ?C2",
			...
			{
				ID:         "C3",
				Expression: "((0 1) (0 1)) complex:exponentiation ?C3",
				Base:       Complex{0, 1},
				Power:      Complex{0, 1},
			...
			{
				ID:         "C4",
				Expression: "((e 0) (-pi/2 0)) complex:exponentiation ?C4",
				Base:       Complex{math.E, 0},
		...
		Inverses: []ComplexInverseInput{
			{
				ID:         "C5",
				Operation:  "asin",
				Expression: "(2 0) complex:asin ?C5",
				Input:      Complex{2, 0},
			...
			},
			{
				ID:         "C6",
				Operation:  "acos",
				Expression: "(2 0) complex:acos ?C6",
				Input:      Complex{2, 0},
	...
	inverseInputs := input.Inverses
	inverses := make([]InverseCase, 0, len(inverseInputs))
	for _, in := range inverseInputs {
		inv := inverseTrigN3(in.ID, in.Operation, in.Expression, in.Input, in.Expected, in.Reason)
